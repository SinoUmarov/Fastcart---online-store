		...
		<div className='my-[30px]'>
			{checkout && (
				<div className='fixed border-[1px] w-[70%] md:w-[30%] left-[15%] md:left-[35%] rounded-[5px] p-[20px] bg-[#fff]'>
					...
					<button
						className='text-[18px] rounded-[5px] border-[1px] w-[100%] mt-[30px]'
			...
			<p className='text-[#808080] my-[30px] w-[90%] m-auto text-[18px]'>
				Product / View Cart / <span className='text-[#000]'> CheckOut</span>
			</p>
			<div className='w-[90%] m-auto flex flex-col md:flex-row md:justify-between items-start'>
				<div className='flex  flex-col gap-y-[10px] w-[100%] md:w-[40%]'>
					...
					<div
						style={{ boxShadow: '1px 1px 5px 2px #EBEBEB' }}
						className='flex rounded-[5px] p-[15px] flex-col gap-y-[20px]'
					>
						<input
							className='border-[1px] p-[10px] rounded-[5px] text-[18px] border-[#C4C4C4]'
							type='text'
				...
				<div className='flex flex-col gap-y-[10px] w-[100%] md:w-[40%]'>
					<div className='flex flex-col gap-y-[10px] w-[100%]'>
						{productsCart.map(el => (
							<div
								key={el.id}
								className='flex justify-between items-center p-[10px] rounded-[5px]'
							>
					...
					<div className='flex mt-[30px] flex-col gap-y-[10px] w-[100%]'>
						<div className='flex flex-row justify-between'>
							<p className='text-red-700'>Subtotal:</p>
							<p>${totalPrice}</p>
						</div>
						<div className='flex flex-row justify-between'>
							<p className='text-red-700'>Shipping:</p>
							<p>Free</p>
						</div>
						<hr />
						<div className='flex flex-row justify-between'>
					...
					<div>
						<div className='flex flex-row gap-x-[10px] items-center mb-2'>
							...
							<label htmlFor='bank' className='text-[18px] font-[600]'>
								Bank
							</label>
						</div>
						<div className='flex flex-row gap-x-[10px] items-center'>
							<input type='radio' id='cod' name='paymentMethod' value='cash' />
							<label htmlFor='cod' className='text-[18px] font-[600]'>
								Cash on delivery
							</label>
						</div>
					</div>
					<div
						className='flex p-[10px] flex-row justify-between w-[100%] my-[20px]'
						style={{ boxShadow: '1px 1px 5px 2px #EBEBEB' }}
					>
						<input
							className='p-[10px] w-[70%] border-[1px] border-[#D0D0D0] rounded-[5px] text-[18px]'
							type='text'
							placeholder='Coupon Code'
						/>
						<button className='text-center border-[1px] text-[18px] font-[500]  border-[#DB4444] rounded-[5px] w-[26%] py-[10px] bg-[#5f1219] text-white hover:shadow-2xl'>
							Apply
						</button>
					</div>
					<button
						onClick={() => dispatch(clearProductCart())}
						className='w-[50%] md:w-[30%] text-center my-[10px] text-[18px] p-[10px] bg-[#DB4444] text-[#fff] rounded-[5px] hover:shadow-2xl '
					>
						Place Order
					</button>
				</div>
